  ...
  where
    ...
    asidSlots = assert (map fst table `isPrefixOf` [1..]) (map snd table)
      where
        table = sortBy (comparing fst)
            [ let asidHigh =
                    fromMaybe
                        (error "--json mode requires that every ASIDPool be assigned an ASID")
                        maybeAsidHigh
    ...
    objects =
        [ NamedObject
            ...
            , object =
                let isVSpace = objId `S.member` vspaces
                    obj = objMap M.! objId
                 in translateObjWith arch translateId obj isVSpace
            }
        | objId <- orderedObjectIds
        ]

    vspaces = S.fromList $ do
        C.TCB { slots } <- M.elems objMap
        C.PTCap { capObj } <- return $ slots M.! C.tcbVTableSlot
        return capObj

translateObjWith :: C.Arch -> (C.ObjID -> ObjID) -> C.KernelObject Word -> Bool -> Object
translateObjWith arch translateId obj isVSpace = case obj of
  ...
  where
    translateCap = translateCapWith translateId
    translateCapTable = M.toList . M.map translateCap . M.mapKeys toInteger

translateCapWith :: (C.ObjID -> ObjID) -> C.Cap -> Cap
translateCapWith translateId cap = case cap of
...
translateName _ = error "object indices are not supported in --json mode"
